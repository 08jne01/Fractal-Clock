...
static inline void rotateInPlacef( Vec2f& v, float angle )
{
	float cosfa = cosf( angle );
	float sinfa = sinf( angle );

	float x = v.x * cosfa - v.y * sinfa;
	float y = v.x * sinfa + v.y * cosfa;
	v.x = x;
	v.y = y;
}
